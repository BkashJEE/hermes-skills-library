...
Recreate uses Hermes Desktop's selected Agent and active workspace to create a normal visible chat, then submits a four-stage request. The Agent first reads the original story as untrusted reference material and maps the workflow, separating sourced facts from assumptions. It then inspects the workspace read-only, asks about the desired recreation, and recommends either the current Agent or a dedicated Bot. A dedicated Bot receives a Bot Forge-style blueprint for its role/SOUL, workspace boundary, skills, plugins, integrations, routines, memory, permissions, observability, tests and rollback. The Agent must stop before the creation stage and wait for approval before creating a Bot, installing software, editing files or configuration, using credentials, scheduling routines, or taking external actions. Approved Bot creation must use Hermes's supported Bot/Profile flow rather than raw profile-directory edits. Browser preview mode shows the explanation but does not create or submit a Hermes chat. Desktop versions without the required routed-session SDK return an update message instead of falling back to an unsafe or incomplete launch.
